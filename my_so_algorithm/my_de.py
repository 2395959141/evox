...
class my_DE(Algorithm):
    def __init__(
        ...
        Pd,
        pop_size,
        base_vector="rand", # 基向量选择策略 ("rand" 或 "best")
        num_difference_vectors=1, # num_difference_vectors: 差分向量的数量
        differential_weight=0.5, # differential_weight: 差分权重
        cross_probability=0.9, # cross_probability: 交叉概率
        batch_size=100, # batch_size: 批次大小
        replace=False, # replace: 是否允许重复选择个体
        mean=None,
        stdvar=None,
    ):
    ...
    def _ask_one(self, index, R, population, best_index, random_choiced, mask):
        ...
        def adjust_solution(solution):
            ...
            def body_fun(state):
                ...
                sol = sol - diff / self.dim
                sol = jnp.clip(sol, self.lb, self.ub)
        ...
        trial_vector = adjust_solution(trial_vector)
        trial_vector = jnp.clip(trial_vector, self.lb, self.ub)
